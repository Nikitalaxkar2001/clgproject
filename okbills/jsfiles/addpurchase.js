...
Database.all('SELECT * FROM suppliers', function (err, rows) {
  rows.forEach(function (row) {
    console.log(row)
    $('#suppliers').append(` <option class="dropdownitems" id="${row.id}" value ="${row.supp_name}"> ${row.supp_name}</option>`)
  });
});
...
$("#suppliers").on('change', function () {
  supplierdetails = ($(this).find('option:selected').attr('id'));
});
$("#suppliers").change(function () {
  Database.all('SELECT * FROM Parties WHERE id = ? ', [supplierdetails], function (err, rows) {
    rows.forEach(function (row) {
      var show = $('#show_supplier_details').html(`<div>Name : -${row.Party}</div><div>Contact :- ${row.PhoneNo}</div><div>Email :- ${row.Email}</div><div>Address :- ${row.Billingaddress}</div>`);
    })
  })
});
