...
function login(){
       ...
       if(user ==""|| pass == ""){
        ...
        if(rows == 0){
          $('#incorrect').show('slow').delay(1000).hide('slow');
           ...
           localStorage.setItem('isIDavaliable',true);
 ...
 console.log('hello');
let cid = localStorage.getItem('companyid');
let editsql = 'UPDATE  company_details SET companyname = ?,address = ?,city = ?,pincode = ?,state = ?,website = ?,email = ?,tax = ?,username = ?,password = ? WHERE id = ?'
db.run( editsql,[ecompany ,eaddress, eweb , eemail ,euser ,epass , estate,ecity ,etax ,cid,epin],function(err,rows){
if(err){
  throw err;
}
else{
